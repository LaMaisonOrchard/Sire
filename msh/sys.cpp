...
int execute  (char const *comm, char **envp)
{
   ...
   //while (isspace(*p)) p++; - This will cause problems for Python
   
   if ('\0' == *p)
   {
      status = 0;
   }
   else if (NULL != (script = makeScript(p)))
   {
      int rtn;
      int i;
      ...
      char *p = (char*)(char const*)deps;
      char *q = p;
            
      argv[0] = (char*)malloc(strlen(script)+1);
      strcpy(argv[0], script);
            
      argv[1] = (char*)malloc(strlen((char const*)target)+1);
      strcpy(argv[1], (char const*)target);
...
int MYspawnve(int wait, char *script, char **argv, char **envp)
{
    RTN_TYPE rtn = 0;
    int      pid = 0;
      ...
      else
      {
         ...
         // that 'sirenv' passes back.
         {
             ...
             if (0 == pipe(pipes))
             {
                ...
                MYspawnve(MY_NOWAIT, script, globalArgv, Tenv::appGetEnvp());
...
Tpipe::Tpipe(char const *comm, int read, Tenv *env) : Dnext(Dhead), Dscript("")
{
   ...
   if ((0 != strlen(comm)) && (0 == pipe(pipes)))
   {
      ...
      // Spawn the sub process
      Dpid = MYspawnve(MY_NOWAIT, (char*)(char const*)Dscript, argv, localEnv);
...
inline int isFile(char const *file)
{
    return (0 == access((char*)file, R_OK | F_OK));
}
