...
public:
   ...
   TstrBuf &cat(char const *text, int start = -1);
...
inline char &TstrBuf::operator [] (int           idx)       {return ((*this)[(unsigned long)idx]);}
inline char  TstrBuf::operator [] (unsigned long idx) const {return ((NULL == Dbuff)?('\0'):(Dbuff[idx]));}
inline char  TstrBuf::operator [] (int           idx) const {return ((NULL == Dbuff)?('\0'):(Dbuff[idx]));}
